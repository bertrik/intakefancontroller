...
void redpress() {                                         //add 15 minutes to time, up to the maximum delay allowed (120 minutes)
...
void greenpress( ){                                       //reset all time variables to zero
 delay(40);                                               //debounce overkill
 if (digitalRead(3) == 0){
  tm.Hour = 0;
  tm.Minute = 0;
...
void countdown() {                                        //subtract count from time and display time/status
...
void turnoff() {                                          //pulse coil to switch fan to OFF, set fanon status to prevent retrigger
 if (fanon == true) {
   digitalWrite(4, HIGH);
   delay(60);
   digitalWrite(4, LOW);
...
void turnon() {                                           //pulse coil to switch fan back on, set fanon status to prevent retrigger
 if (fanon == false) {
   display.setSegments(ON);
   digitalWrite(5, HIGH);
   delay(60);
